...
public class TestUtils {
  ...
  private static boolean atLeastVersion(String expectedVersion, Connection connection) {
    ...
    try {
      currentVersion = currentVersion(connection.getServerProperties().get("version").toString());
  ...
  public static boolean queueExists(final String queue, final Connection connection)
      throws Exception {
  ...
  public static boolean exchangeExists(final String exchange, final Connection connection)
      throws Exception {
    return resourceExists(
        () -> {
          Channel channel = connection.createChannel();
        ...
        public void evaluate() throws Throwable {
          try {
            ...
            throw new AssumptionViolatedException(
                "Could not check whether broker is running on Docker or not", e);
          }
          base.evaluate();
        }
      };
