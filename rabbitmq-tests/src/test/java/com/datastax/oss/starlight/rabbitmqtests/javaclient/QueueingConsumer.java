...
package com.datastax.oss.starlight.rabbitmqtests.javaclient;
 ...
 * It is meant to be using in tests.
 *
 * <p>Deprecated in favor of {@link DefaultConsumer} (see below for background). Will be removed in
 * next major release.
 *
 * <p>The general pattern for using QueueingConsumer is as follows:
 *
 * <pre>
 ...
 *
 * <p><code>QueueingConsumer</code> was introduced to allow applications to overcome a limitation in
 * the way <code>Connection</code> managed threads and consumer dispatching. When <code>
 * QueueingConsumer</code> was introduced, callbacks to <code>Consumers</code> were made on the
 * <code>Connection's</code> thread. This had two main drawbacks. Firstly, the <code>Consumer</code>
 * could stall the processing of all <code>Channels</code> on the <code>Connection</code>. Secondly,
 * if a <code>Consumer</code> made a recursive synchronous call into its <code>Channel</code> the
 * client would deadlock.
 *
 * <p><code>QueueingConsumer</code> provided client code with an easy way to obviate this problem by
 * queueing incoming messages and processing them on a separate, application-managed thread.
 *
 * <p>The threading behaviour of <code>Connection</code> and <code>Channel</code> has been changed
 * so that each <code>Channel</code> uses a distinct thread for dispatching to <code>Consumers
 * </code>. This prevents <code>Consumers</code> on one <code>Channel</code> holding up <code>
 * Consumers</code> on another and it also prevents recursive calls from deadlocking the client. As
 * such, it is now safe to implement <code>Consumer</code> directly or to extend <code>
 * DefaultConsumer</code> and <code>QueueingConsumer</code> is a lot less relevant.
 */
public class QueueingConsumer extends DefaultConsumer {
  ...
  private Delivery handle(Delivery delivery) {
    if (delivery == POISON || delivery == null && (_shutdown != null || _cancelled != null)) {
      if (delivery == POISON) {
        _queue.add(POISON);
