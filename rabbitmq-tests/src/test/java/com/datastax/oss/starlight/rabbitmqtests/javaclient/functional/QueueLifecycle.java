...
public class QueueLifecycle extends BrokerTestCase {
   ...
   */
  void verifyQueue(
      String name, boolean durable, boolean exclusive, boolean autoDelete, Map<String, Object> args)
      throws IOException {
  ...
  void verifyNotEquivalent(boolean durable, boolean exclusive, boolean autoDelete)
      throws IOException {
    String q = "queue";
    ...
    try {
      ...
      if (exclusive) checkShutdownSignal(AMQP.RESOURCE_LOCKED, ioe);
      else checkShutdownSignal(AMQP.PRECONDITION_FAILED, ioe);
      return;
    }
  ...
  /**
   * From amqp-0-9-1.xml, for "passive" property, "equivalent" rule: "If not set and the queue
   * exists, the server MUST check that the existing queue has the same values for durable,
   * exclusive, auto-delete, and arguments fields. The server MUST respond with Declare-Ok if the
  ...
  @Test
  @Ignore("Need to implement autodelete queue first")
  ...
  @Test
  @Ignore("Need to implement exclusive queues first")
