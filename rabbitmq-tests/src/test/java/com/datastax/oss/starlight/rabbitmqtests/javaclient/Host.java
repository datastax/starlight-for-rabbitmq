...
public class Host {
  ...
  public static Process rabbitmqctlIgnoreErrors(String command) throws IOException {
    return executeCommandIgnoringErrors(
        rabbitmqctlCommand() + rabbitmqctlNodenameArgument() + " " + command);
  }
  ...
  public static List<ConnectionInfo> listConnections() throws IOException {
    String output =
        capture(
            rabbitmqctl("list_connections -q pid peer_port client_properties").getInputStream());
  ...
  private static Host.ConnectionInfo findConnectionInfoFor(
      List<Host.ConnectionInfo> xs, NetworkConnection c) {
    Connection conn = (Connection) c;
    Predicate<ConnectionInfo> predicate =
        conn.getClientProvidedName() == null
            ? ci -> ci.getPeerPort() == c.getLocalPort()
            : ci ->
                ci.hasClientProvidedName()
                    && ci.getClientProvidedName().equals(conn.getClientProvidedName());
