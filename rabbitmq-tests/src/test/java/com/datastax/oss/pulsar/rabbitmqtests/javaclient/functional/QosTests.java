...
import com.datastax.oss.pulsar.rabbitmqtests.javaclient.QueueingConsumer.Delivery;
...
public class QosTests extends BrokerTestCase {
  ...
  public void fill(int n) throws IOException {
    for (int i = 0; i < n; i++) {
      channel.basicPublish("amq.fanout", "", null, Integer.toString(i).getBytes());
    }
  }
  ...
  // TODO: RMQ doesn't support prefetch size but Qpid does
  // @Test
  public void messageLimitPrefetchSizeFails() throws IOException {
    try {
  ...
  public void permutations() throws IOException {
    ...
    for (int limit : Arrays.asList(1, 2)) {
      for (boolean multiAck : Arrays.asList(false, true)) {
        // TODO: reactivate when transactions are supported
        // for (boolean txMode : Arrays.asList(true, false)) {
        for (boolean txMode : Arrays.asList(false)) {
  ...
  protected void runLimitTests(int limit, boolean multiAck, boolean txMode, int queueCount)
      throws IOException {
    ...
    for (String q : queues) {
      GetResponse r = TestUtils.basicGet(channel, q, false);
      assertNotNull(r);
  ...
  protected Delivery ack(List<Delivery> d, boolean multiAck) throws IOException {
    Delivery last = null;
  ...
  protected List<String> configure(QueueingConsumer c, int limit, int queueCount, int messages)
      throws IOException {
  ...
  protected List<Delivery> configure(QueueingConsumer c, int limit, int messages)
      throws IOException {
    channel.basicQos(limit, true);
    declareBindConsume(c);
    fill(messages);
  ...
  protected String declareBindConsume(QueueingConsumer c) throws IOException {
    return declareBindConsume(channel, c, false);
  }

  protected String declareBindConsume(Channel ch, QueueingConsumer c, boolean noAck)
      throws IOException {
  ...
  protected void ackDelivery(Delivery d, boolean multiple) throws IOException {
    ackDelivery(channel, d, multiple);
  }

  protected void ackDelivery(Channel ch, Delivery d, boolean multiple) throws IOException {
