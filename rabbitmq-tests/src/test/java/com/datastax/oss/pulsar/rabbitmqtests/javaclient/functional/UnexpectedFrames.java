        ...
        new Confuser() {
          public Frame confuse(Frame frame) {
            if (frame.type == AMQP.FRAME_HEADER) {
              ...
              Frame confusedFrame = new Frame(AMQP.FRAME_HEADER, frame.channel, payload);
  ...
  public void heartbeatOnChannel() throws IOException {
    expectUnexpectedFrameError(
        new Confuser() {
          public Frame confuse(Frame frame) {
            if (frame.type == AMQP.FRAME_METHOD) {
              return new Frame(AMQP.FRAME_HEARTBEAT, frame.channel + 1);
            }
            return frame;
          }
        });
  ...
  public void unknownFrameType() throws IOException {
    expectError(
        ...
        new Confuser() {
          public Frame confuse(Frame frame) {
            if (frame.type == AMQP.FRAME_METHOD) {
              return new Frame(0, frame.channel, "1234567890\0001234567890".getBytes());
            }
            return frame;
          }
        });
