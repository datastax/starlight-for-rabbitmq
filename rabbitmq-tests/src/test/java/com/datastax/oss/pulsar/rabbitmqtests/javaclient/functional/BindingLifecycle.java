 ...
 *
 * <p>The tests attempt to declare durable queues on a secondary node, if present, and that node is
 * restarted as part of the tests while the primary node is still running. That way we exercise any
 * node-down handler code in the server.
 */
public class BindingLifecycle extends BindingLifecycleBase {

  /** This tests that when you purge a queue, all of its messages go. */
  @Test
  @Ignore("Need to implement queue purge first")
  public void queuePurge() throws IOException {
    ...
    GetResponse response = TestUtils.basicGet(channel, binding.q, true);
  ...
  /**
   * See bug 21854: "When Queue.Purge is called, sent-but-unacknowledged messages are no longer
   * purged, even if the channel they were sent down is not (Tx-)transacted."
   */
  ...
  public void unackedPurge() throws IOException {
    ...
    GetResponse response = TestUtils.basicGet(channel, binding.q, false);
    ...
    // recover.
    channel.queuePurge(binding.q);
    response = TestUtils.basicGet(channel, binding.q, true);
    assertNull("The response SHOULD BE null", response);

    channel.basicRecover();
    response = TestUtils.basicGet(channel, binding.q, false);
    channel.basicRecover();
  ...
  /**
   * This tests whether when you delete an exchange, that any bindings attached to it are deleted as
   * well.
   */
  @Test
  ...
  /**
   * This tests whether the server checks that an exchange is actually being used when you try to
   * delete it with the ifunused flag.
   *
   * <p>To test this, you try to delete an exchange with a queue still bound to it and expect the
   * delete operation to fail.
   */
  @Test
  @Ignore("Need to implement exchange delete ifUnused first")
  ...
  /**
   * This tests whether the server checks that an auto_delete exchange actually deletes the bindings
   * attached to it when it is deleted.
   *
   * <p>To test this, you declare and auto_delete exchange and bind an auto_delete queue to it.
   *
   * <p>Start a consumer on this queue, send a message, let it get consumed and then cancel the
   * consumer
   *
   * <p>The unsubscribe should cause the queue to auto_delete, which in turn should cause the
   * exchange to auto_delete.
   *
   * <p>Then re-declare the queue again and try to rebind it to the same exchange.
   *
   * <p>Because the exchange has been auto-deleted, the bind operation should fail.
   */
  @Test
  @Ignore("Need to implement exchange autodelete first")
  public void exchangeAutoDelete() throws IOException, TimeoutException {
    doAutoDelete(false, 1);
  }

  /**
   * Runs something similar to testExchangeAutoDelete, but adds different queues with the same
   * binding to the same exchange.
   *
   * <p>The difference should be that the original exchange should not get auto-deleted
   */
  @Test
  @Ignore("Need to implement exchange autodelete first")
  ...
  public void testUnbind(String exchange) throws Exception {
    Binding b = new Binding(channel.queueDeclare().getQueue(), exchange, "quay");
